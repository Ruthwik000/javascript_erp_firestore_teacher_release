...
REM Update index.html to use obfuscated version
if exist "index.html" (
    echo Updating index.html to reference app.obfuscated.js...
    powershell -NoProfile -ExecutionPolicy Bypass -Command "$content = Get-Content 'index.html' -Raw; $content = $content -replace '<script src=\"app\.js\"></script>', '<script src=\"app.obfuscated.js\"></script>'; Set-Content 'index.html' $content"
    echo [92mindex.html updated[0m
    echo.
)
